...
module world::authority_tests {
    ...
    use world::{authority, world};

    #[test]
    fun test_mint_and_burn_admin_cap() {
        let _governor = @0xA;
        let _admin = @0xB;
        ...
        {
            let gov_cap = ts::take_from_sender<world::GovernorCap>(&ts);
            let admin_cap = ts::take_from_address<authority::AdminCap>(&ts, _admin);

            authority::burn_admin_cap(&gov_cap, admin_cap);
